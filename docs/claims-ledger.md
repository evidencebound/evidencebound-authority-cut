...
| model-callable Strands tools cannot approve/revoke authority | source + unit test + public Strands loop | VERIFIED |
| premature first-funds approval fails closed | unit/API test | VERIFIED locally + public CI |
| correction rolls back 6 executed reversible protected effects in controlled path | evaluation + HTTP smoke + public Strands loop | VERIFIED |
...
| real Strands Agent loop executes all three published tools with deterministic custom Model provider | public GitHub Actions trace | VERIFIED |
| a foundation-model-backed Strands agent executes the full flow | external model invocation / trace | UNVERIFIED / BLOCKED |

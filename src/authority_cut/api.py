...
app = FastAPI(title="Authority Cut", version="0.2.0")
...
@app.post("/api/evidence/vendor-risk/supported")
def supported_vendor_risk_evidence() -> dict:
    p = plane()
    p.apply_evidence(
        "vendor-risk",
        evaluate_bank_ecp_bridge("DORA_VENDOR_RESPONSIBILITY_SUPPORTED"),
...
@app.post("/api/evidence/vendor-risk/corrected")
def corrected_vendor_risk_evidence() -> dict:
    p = plane()
    affected = p.apply_evidence(
        "vendor-risk",
        evaluate_bank_ecp_bridge("DORA_VENDOR_RESPONSIBILITY_REVERSED"),
    )
    out = snapshot(p)
    out["affected"] = sorted(affected)
    out["correction_trigger"] = "BANK_ECP_EVIDENCE_CHANGE"
    return out
